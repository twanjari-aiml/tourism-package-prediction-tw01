...
# Programmatically identify numerical and categorical features
numerical_cols = Xtrain.select_dtypes(include=['number']).columns.tolist()
categorical_cols = Xtrain.select_dtypes(include=['object', 'category']).columns.tolist()

# Exclude binary columns from categorical if they are already handled as numerical
binary_cols = [col for col in numerical_cols if Xtrain[col].nunique() <= 2 and set(Xtrain[col].dropna().unique()).issubset({0, 1})]

numeric_features = [col for col in numerical_cols if col not in binary_cols]
categorical_features = categorical_cols + binary_cols # Treat binary numerical as categorical for OneHotEncoder
...
with mlflow.start_run():
    ...
    model_path = "tourism_project/deployment/best_tourism_package_predictor_model_v1.joblib"
